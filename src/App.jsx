...
function Dashboard({ drafts, onOpen, onCreate, onDelete, onClearDrafts, onUpdate }) {
  const todayDate = today();
  const confirm = useConfirm();
  const toast = useToast();
  // الأحدث أولاً — لو نفس اليوم نرتب بـ createdAt
  ...
  const saveEdit = (d) => {
    if (editingName.trim()) {
      onUpdate({ ...d, name: editingName.trim() });
            ...
            >
              ...
              {editingRegionId === r.id ? (
                ...
                <span
                  ...
                  style={{ flex: 1, minWidth: 0, fontSize: 13, fontWeight: 700, cursor: 'text' }}
                >{r.name}</span>
              )}
              {/* ثابتان دايماً على اليسار */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                ...
                <button
                  className="btn btn-ghost btn-icon btn-sm"
                  style={{ color: 'var(--red-dim)', flexShrink: 0 }}
                  onClick={() => del(r.id)}
                  title="حذف المنطقة"
                >🗑️</button>
              </div>
